...
<head>
    ...
    <title>{{ __('app.login.title') }} — {{ $site->trans('site_name') ?? 'WhatsAppBizAI' }}</title>
    ...
    <div class="card">
        ...
        <form method="POST" action="{{ url('login') }}">
            ...
            <div class="form-group">
                <label>{{ __('app.login.email') }}</label>
            ...
            <div class="remember-row">
                ...
                <label for="remember">{{ __('app.login.remember') }}</label>
            </div>

            <button type="submit" class="btn">{{ __('app.login.submit') }}</button>
        </form>

        <div style="position:relative;text-align:center;margin:20px 0;">
            <hr class="divider">
            <span style="position:absolute;top:-8px;left:50%;transform:translateX(-50%);background:#fff;padding:0 10px;font-size:12px;color:#94a3b8;">{{ __('app.login.or') }}</span>
        </div>
        ...
        <div class="register-link">
            {{ __('app.login.no_account') }} <a href="{{ url('register') }}">{{ __('app.login.create_account') }}</a>
        </div>

        <hr class="divider">

        <div class="admin-link">
            <a href="/admin/login">{{ __('app.login.admin') }}</a>
        </div>
    </div>
