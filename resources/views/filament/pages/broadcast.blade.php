<x-filament-panels::page>
    ...
    <div class="mt-8 rounded-xl border border-gray-200 bg-gray-50 dark:bg-gray-900 dark:border-gray-700 p-5 text-sm text-gray-500">
        ...
        <p class="mt-3">Variables: <code class="bg-gray-200 dark:bg-gray-800 px-1 rounded">@php echo '{{nom}}' @endphp</code>, <code class="bg-gray-200 dark:bg-gray-800 px-1 rounded">@php echo '{{prenom}}' @endphp</code>, <code class="bg-gray-200 dark:bg-gray-800 px-1 rounded">@php echo '{{entreprise}}' @endphp</code></p>
    ...
    >
        <div style="width:100%;max-width:720px;background:#fff;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,.3);overflow:hidden;">
            ...
            <div style="padding:20px;">
                <iframe id="bcast-iframe" style="width:100%;min-height:400px;border:1px solid #e2e8f0;border-radius:8px;" title="Aperçu message"></iframe>
            </div>
    ...
    <script>
        $wire.on('previewContent', () => {
            setTimeout(() => {
                ...
                var iframe = document.getElementById('bcast-iframe');
                if (iframe && html) {
                    iframe.srcdoc = html;
                }
            }, 150);
